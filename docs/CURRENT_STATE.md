...
| Semantic authority — UNKNOWN triage | **2026-08-14.** All 18 remaining UNKNOWN detectors (111 model-route lessons) triaged and ranked; **0 promoted to blocking**, deliberately. The strongest candidate (generic answer-class) measured 0 false blocks on 21,120 deterministic packages but **48 / 3,287 (1.46 %) on model-authored live packages** — every one a false positive — so it is not wired. Blocking coverage stays **43**. The gating constraint is corpus, not ideas: across all 18 detectors the known-good model-authored corpus holds **13 packages**, and 13 of 18 detectors have none. `data/semantic_authority_status.json` now publishes real runtime authority per lesson so `enforcement_mode: blocking` cannot be misread. Best next candidate: `scientific_notation` (`EXACT_PARSED_MATH`, 1 lesson) once a live corpus exists. |
| Semantic authority | **Made honest 2026-08-14.** All 354 lesson contracts were labelled `blocking` while only 2 of 41 detectors existed — and both covered *deterministic* lessons only, where drift is structurally impossible, so effective blocking coverage on the model route was **0**. One derived generic primitive (measured-quantity dimension) now covers **54 model-route lessons, 43 of them discriminating**, proven at 0 false positives over 39,384 known-good packages and 100 % detection of 667 mutations. The other 18 model-route detectors are reported as `UNKNOWN` rather than counted as protection. See [ARCHITECTURE.md](ARCHITECTURE.md) and `scripts/build_semantic_authority_report.py`. |
