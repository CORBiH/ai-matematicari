...
def test_empty_answer_retried_with_higher_cap(master, tmap):
    ...
    # retry ide sa uvećanim budžetom do kapa (explain baza 1400 → cap 2400)
    assert chat.calls["max_tokens"] == [1400, 2400]
...
def test_quick_mode_cap_raised(master, tmap):
    # BUG 14 + live nalaz (2026-07-10): reasoning model troši budžet na
    # razmišljanje pa je niska baza pucala na length; quick baza podignuta na 900.
    chat = _seq_chat([_resp("4")])
    svc.handle_chat(_payload("quick"), chat, master, tmap, model="m", timeout=1)
    assert chat.calls["max_tokens"][0] == 900
